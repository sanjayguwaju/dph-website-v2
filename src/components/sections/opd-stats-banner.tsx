  ...
  const groups = [
    {
      ...
      items: [
        { label: t("male"), value: stats.opdMale ?? 0, icon: <User size={18} />, color: "#2563eb" },
        { label: t("female"), value: stats.opdFemale ?? 0, icon: <Users size={18} />, color: "#db2777" },
        { label: t("total"), value: stats.opdTotal ?? 0, icon: <Activity size={18} />, color: "#059669" },
      ]
    },
    {
      title: t("emergencyServiceRecipients"),
      items: [
        { label: t("male"), value: stats.inpatientMale ?? 0, icon: <User size={18} />, color: "#2563eb" },
        { label: t("female"), value: stats.inpatientFemale ?? 0, icon: <Users size={18} />, color: "#db2777" },
        { label: t("total"), value: stats.inpatientTotal ?? 0, icon: <Activity size={18} />, color: "#059669" },
      ]
    },
    {
      title: t("admittedPatients"),
      items: [
        { label: t("total"), value: stats.totalBeds ?? 0, icon: <Bed size={22} />, color: "#7c3aed" },
      ]
    },
    {
      title: t("healthInsuranceProgram"),
      items: [
        { label: t("total"), value: stats.bedOccupancy ?? 0, icon: <HeartPulse size={22} />, color: "#0d9488" },
      ]
    }
  ];
    ...
    <section className="stats-dashboard-simple container-refined mt-8 mb-12">
      <div className="section-header-v2">
        <h2 className="section-heading-v2">
          {t("statsTitle")} <span className="text-gray-500 text-lg font-normal ml-2">{lastUpdateStr ? `(${lastUpdateStr})` : ""}</span>
        </h2>
      </div>
